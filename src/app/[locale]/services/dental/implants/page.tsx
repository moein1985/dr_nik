...
export default async function DentalImplantsPage({ params }: Props) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();

  const localeCode = locale as Locale;
  const items = serviceMediaManifest.dental.implants.map((images, index) => ({
    ...
    <main className="bg-slate-50 text-slate-900">
      <section className="bg-clinic-teal px-4 py-14 text-white lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="mt-3 text-3xl font-bold lg:text-5xl">Implants</h1>
          <p className="mt-4 max-w-3xl text-base text-white/80">Reusable carousel for dental implant patient imagery and case references.</p>
        </div>
      </section>
